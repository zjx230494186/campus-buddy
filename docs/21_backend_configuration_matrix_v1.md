...
|---------|------|--------|----------|----------|
| `local` | 开发者本机开发 | 不连接 | no-op 替身 | 进程内内存 |
| `test` | 自动化测试 | 不连接 | no-op 替身 | 进程内内存 |
| `deploy` | ECS 部署/演示 | PostgreSQL（环境变量） | 需替换为真实实现 | PostgreSQL |
...
|--------|-------|------|--------|------|
| `campus-buddy.campus-email.allowed-domains[0]` | `campus.edu.cn` | `campus.edu.cn` | `campus.edu.cn` | 校园邮箱域名白名单；deploy 需替换为真实学校域名 |
...
- `CampusBuddyProperties`（前缀 `campus-buddy`）
  - `CampusEmail`：`allowedDomains`、`codeExpiresInSeconds`、`resendAfterSeconds`
...
- 邮件发送：当前为 `NoopCampusEmailVerificationCodeSender`，后续需配置化替换
